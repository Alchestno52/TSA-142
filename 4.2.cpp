...
/**
 * @brief Вводим значения с клавиатуры с проверкой на валидность
 * @return Возвращаем значение, если оно правильное, иначе завершает программу
 */
double getValue();
...
/**
 * @brief Функция для проверки, что значение x входит в область определения функции (0;+∞)
 * @param x значение x
 * @return true, если x входит в (0;+∞), иначе false
 */
...
int main()
{
    ...
    for (double x = startX; x <= endX + numeric_limits<double>::epsilon(); x += step)
    {
        ...
        {
            cout << "Значение x = " << x << " не принадлежит области определения функции (0;+∞)" << endl;
            return 0;
        }
    }
    return 0;
