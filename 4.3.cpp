...
/**
 * @brief Функция для проверки, что значение x не равно нулю (по модулю)
 * @param x значение x
...
int main()
{
    ...
    for (double x = startX; x <= endX + numeric_limits<double>::epsilon(); x += step)
    {
        ...
        {
            cout << "Значение x = " << x << " не принадлежит области определения функции (x != 0)" << endl;
            return 0;
        }
    }
    return 0;
...
bool isOOF(const double x)
{
    return abs(x) > numeric_limits<double>::epsilon();
}
